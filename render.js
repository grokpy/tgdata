...
export function render() {
    try {
        if (!canvas || !ctx) {
            console.error('Canvas or context is undefined in render');
            return;
        }
                ...
                if (tile) {
                    ...
                    if (tile.bonusType === 'horizontal_arrow') {
                        ...
                        switch (state.selectedShapes[tile.type]) {
                            ...
                            case 'diamond':
                                ctx.beginPath();
                                ctx.moveTo(x, y - size / 2); // Верхняя точка
                                ctx.lineTo(x + size / 2, y); // Правая точка
                                ctx.lineTo(x, y + size / 2); // Нижняя точка
                                ctx.lineTo(x - size / 2, y); // Левая точка
                                ctx.closePath();
                                ctx.fill();
                                break;
                            case 'pentagon':
                                ctx.beginPath();
                                const angle = (Math.PI * 2) / 5; // Угол между вершинами пентагона
                                for (let i = 0; i < 5; i++) {
                                    const px = x + (size / 2) * Math.cos(angle * i - Math.PI / 2);
                                    const py = y + (size / 2) * Math.sin(angle * i - Math.PI / 2);
                                    if (i === 0) {
                                        ctx.moveTo(px, py);
                                    } else {
                                        ctx.lineTo(px, py);
