    ...
    while (matches) {
        ...
        matches.forEach(match => {
            ...
            if (match.length === 4) {
                ...
                if (match.direction === 'l-shaped' && !bonusStarPlaced) {
                    bonusType = 'bonus_star';
                    bonusPos = match.intersection;
                    bonusStarPlaced = true;
                } else if (match.direction === 'horizontal' && !bonusStarPlaced) {
                    bonusType = 'bonus_star';
                    // Сортируем по столбцу и выбираем среднюю позицию
                    const sortedPositions = match.positions.sort((a, b) => a.col - b.col);
                    const middleIndex = Math.floor(match.length / 2);
                    bonusPos = sortedPositions[middleIndex];
                    bonusStarPlaced = true;
                } else if (match.direction === 'vertical' && !bonusStarPlaced) {
                    bonusType = 'bonus_star';
                    // Сортируем по строке и выбираем среднюю позицию
                    const sortedPositions = match.positions.sort((a, b) => a.row - b.row);
                    const middleIndex = Math.floor(match.length / 2);
                    bonusPos = sortedPositions[middleIndex];
                    bonusStarPlaced = true;
                }
