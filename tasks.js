...
export function loadTask() {
    try {
        ...
        if (state.currentTaskIndex < state.predefinedTasks.length) {
            ...
            // Обновляем selectedShapes на основе allowedShapes из текущей задачи
            state.selectedShapes = state.task.allowedShapes || state.ALL_SHAPES;
            // Обновляем selectedColors, чтобы соответствовать количеству фигур
            state.selectedColors = state.ALL_COLORS.slice(0, state.selectedShapes.length);
            console.log(`Loaded predefined task ${state.currentTaskIndex + 1}: Collect ${state.task.count} ${state.task.shape} in ${state.movesLeft} moves with shapes: ${state.selectedShapes.join(', ')}`);
        } else {
            generateNewTask();
        }
        state.collectedShapes = { [state.task.shape]: 0 };
        state.taskScore = 0;
        // Перегенерируем игровое поле с новым набором фигур
        initBoard();
...
export function generateNewTask() {
    try {
        // Определяем два возможных набора фигур для случайных задач
        const shapeSets = [
            ['square', 'circle', 'triangle', 'diamond'], // Набор 1
            ['diamond', 'circle', 'pentagon'] // Набор 2
        ];
        const selectedShapeSet = shapeSets[Math.floor(Math.random() * shapeSets.length)];
        state.selectedShapes = selectedShapeSet;
        state.selectedColors = state.ALL_COLORS.slice(0, state.selectedShapes.length);
        const shape = state.selectedShapes[Math.floor(Math.random() * state.selectedShapes.length)];
        state.task = {
            shape,
        ...
        state.taskScore = 0;
        console.log(`New random task: Collect ${state.task.count} ${state.task.shape} in ${state.movesLeft} moves with shapes: ${state.selectedShapes.join(', ')}`);
